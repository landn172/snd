...
// 主要slider
const mainSlider = new Islider(
  $mainContainer[0],
  [].concat(pageData).reverse(),
  {
    isLooping: false,
    isVertical: 0,
    plugins: [],
...
$back.on("click", () => {
  ...
  const { lastIndex, lastSlider, backHistorys } = lastHistory;
  console.log(lastIndex, lastSlider.container || lastSlider.outer.parentElement);
...
Object.keys(eventConfig).forEach(key =>
  clickEventHandle(key, eventConfig[key])
);

// 重新初始化 sectionSlider
function reInitSectionSlider(data, index = 0) {
  if (sectionSlider && sectionSlider.lastHistory) {
    sectionSlider.lastHistory.destory();
    sectionSlider.lastHistory = null;
  }
  sectionSlider = new TempSlider({ data, index });
